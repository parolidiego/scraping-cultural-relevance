...
Although this passage is not required, it allows websites to contact you if you are causing problems to them with your requests
...
Again with the purpose of displaying information for each day we decided to scrape a table containing the best selling movie of that day in the USA for each day of a year. Also in this case we were able to loop over several years as there was an indication in the URL of which year was displayed. 
...
We have run this function to make repeated calls (always within the limits) to the API for several days. Getting data from ... to ...
...
For The Guardian we applied a similar reasoning as for The New York Times. However this time, each API calls gets back up to 50 results. Therefore we expanded our query window to 5 days instead of 2 days.

## Setting start and end date

The function is structured as before with only minor changes to format dates as need for the API call.
...
With the data that we got we have built a Shiny app where one can choose a date and the data for that date will be displayed.

You can find in this directory file `shiny_app.R`